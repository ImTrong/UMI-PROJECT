  ...
  const handleSubmit = async () => {
    ...
    try {
      const formattedAnswers = Object.keys(answers).map((qId) => {
        ...
        return {
          questionId: qId,
          answerValue: Array.isArray(val) ? val : [val],
        };
      });
    ...
    return (
      <div className="max-w-3xl mx-auto p-4 md:p-8">
        <div className={`card text-center mb-8 border-t-4 ${result.passed ? 'border-green-500' : 'border-red-500'}`}>
          ...
          <p className="text-xl">Điểm của bạn: <span className="font-bold">{result.score}</span> / {result.maxScore}</p>
        ...
        <div className="space-y-6">
          {result.explanations.map((exp: any, index: number) => (
            <div key={exp.questionId} className={`card ${exp.isCorrect ? 'bg-green-50 border-green-200' : 'bg-red-50 border-red-200'}`}>
              <div className="flex gap-3">
                <div className="mt-1">
                  {exp.isCorrect ? <FiCheckCircle className="text-green-600" /> : <FiXCircle className="text-red-600" />}
                </div>
                <div>
                  <p className="font-medium text-gray-900 mb-2">Câu {index + 1}: {quiz.questions.find(q => q.id === exp.questionId)?.questionText}</p>
                  <div className="text-sm">
                    <p><span className="font-semibold text-gray-700">Câu trả lời của bạn:</span> {exp.userAnswers.join(', ')}</p>
                    {!exp.isCorrect && (
                      <p><span className="font-semibold text-green-700">Đáp án chuẩn:</span> {exp.correctAnswers.join(', ')}</p>
                    )}
                  </div>
                  {exp.explanation && (
                ...
                <label key={opt.id} className="flex items-start gap-3 p-3 border rounded-lg hover:bg-gray-50 cursor-pointer transition-colors">
                  <input
                    ...
                    checked={q.questionType === 'MULTIPLE_CHOICE' 
                      ...
                      : answers[q.id] === opt.id}
                    onChange={() => handleAnswerChange(q.id, opt.id, q.questionType)}
                  />
