...
export const AssignmentBuilder = ({ courseId, lessonId, onClose }: AssignmentBuilderProps) => {
  ...
  if (loading) return <div className="p-8 text-center text-gray-500 animate-pulse">Đang nạp cấu hình bài tập thực hành...</div>;

  return (
    <div className="bg-white border rounded-xl shadow-lg mt-4 mb-8 border-primary-200 overflow-hidden">
      {/* HEADER SECTION */}
      <div className="bg-gradient-to-r from-amber-600 to-orange-600 p-5 flex justify-between items-center text-white">
        ...
        <div className="flex items-center gap-3">
          {assignment && (
            <a href={`/instructor/assignments/${assignment.id}/submissions`} target="_blank" rel="noreferrer" className="text-sm font-medium text-amber-900 bg-white/90 hover:bg-white px-3 py-1.5 rounded-lg transition-colors flex items-center gap-1.5 shadow-sm">
              <span>Chấm điểm</span>
              <span className="bg-amber-100 text-amber-700 px-1.5 rounded-md text-xs">Phím tắt</span>
            </a>
          )}
      ...
      <form onSubmit={handleSave} className="p-6 md:p-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-8">
          ...
          <div className="md:col-span-8 space-y-6">
            <div>
              <label className="block text-sm font-semibold text-gray-800 mb-1.5">Tiêu đề Bài tập <span className="text-red-500">*</span></label>
            ...
            <div className="bg-blue-50/50 p-5 rounded-xl border border-blue-100/50">
              <label className="block text-sm font-semibold text-blue-900 mb-1.5">Hướng dẫn từng bước (Tùy chọn)</label>
              <textarea value={instructions} onChange={e => setInstructions(e.target.value)} className="input-field shadow-sm border-blue-200 focus:border-blue-400 focus:ring-blue-400 min-h-[120px] bg-white/70" placeholder="Bước 1: Tải template đính kèm...&#10;Bước 2: Điền thông số...&#10;Bước 3: Lên báo cáo mật độ..." />
              <p className="text-xs text-blue-600/70 mt-2">Phần này sẽ hiện dưới dạng danh sách nhiệm vụ nếu có đánh dấu hoa thị/gạch đầu dòng.</p>
            </div>
          </div>
          ...
          <div className="md:col-span-4 space-y-6">
            <div className="bg-gray-50 rounded-xl p-5 border border-gray-200">
               <h4 className="text-sm font-bold text-gray-800 uppercase tracking-wider mb-4 border-b pb-2">Cấu hình Nộp bài</h4>
               
               <div className="space-y-5">
                  <div>
                    <label className="block text-sm font-semibold text-gray-700 mb-1.5">Điểm tuyệt đối tối đa <span className="text-red-500">*</span></label>
                    <div className="relative">
                      ...
                      <span className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-400 font-bold">Điểm</span>
                    </div>
                  </div>
                  
                  <div>
                    <label className="block text-sm font-semibold text-gray-700 mb-1.5 flex items-center gap-1">
                      <FiClock className="text-gray-400" /> Hạn chót (Due Date)
                    </label>
                  ...
                  <div className="flex items-start gap-3 bg-white p-3 border border-gray-200 rounded-lg shadow-sm">
                    ...
                    <div className="flex flex-col">
                      <label htmlFor="late-sub" className="text-sm font-semibold text-gray-800 cursor-pointer">Cho phép nộp trễ</label>
                      <span className="text-xs text-gray-500 mt-0.5 leading-relaxed">Nếu tắt, cổng nộp sẽ tự động khóa cứng sau hạn chót thời gian trên.</span>
                    </div>
                  </div>
               </div>
            </div>
            ...
            <div className="bg-gray-50 rounded-xl p-5 border border-gray-200">
               <h4 className="text-sm font-bold text-gray-800 uppercase tracking-wider mb-4 border-b pb-2 flex justify-between items-center">
                  Định dạng File
                  <span className="text-xs bg-gray-200 text-gray-600 px-2 py-0.5 rounded-full font-medium">{allowedFileTypes.length} loại</span>
               </h4>
               ...
               <div className="space-y-4">
                  <div className="relative">
                    <input 
                      ...
                      className="input-field shadow-sm pr-10 border-gray-300 focus:border-amber-400 focus:ring-amber-400" 
                      placeholder="Nhập đuôi file (VD: mp4) + Enter..." 
                    />
                    <button type="button" onClick={(e) => handleAddFileType(e as any)} className="absolute right-2 top-1/2 -translate-y-1/2 text-gray-400 hover:text-amber-600 bg-gray-100 hover:bg-amber-50 p-1 rounded-md transition-colors">
                      <FiPlus />
                    </button>
                  </div>
                  
                  <div className="flex flex-wrap gap-2 min-h-[4rem] p-3 bg-white border border-gray-200 rounded-lg shadow-inner content-start">
                    {allowedFileTypes.length === 0 && (
                      <span className="text-xs text-gray-400 italic">Chưa có giới hạn định dạng</span>
                    )}
                    {allowedFileTypes.map(type => (
                      <span key={type} className="inline-flex items-center gap-1 px-2.5 py-1 rounded-md text-xs font-semibold bg-amber-100 text-amber-800 border border-amber-200">
                        .{type}
                  ...
                  <div className="pt-2">
                    <p className="text-xs text-gray-500 font-medium mb-2">Đề xuất nhanh:</p>
                    <div className="flex flex-wrap gap-1.5">
                       {COMMON_FILE_TYPES.filter(t => !allowedFileTypes.includes(t)).slice(0, 5).map(type => (
                         <button
                           ...
                           className="text-[10px] font-medium text-gray-500 hover:text-amber-700 bg-gray-100 hover:bg-amber-50 border border-gray-200 hover:border-amber-200 rounded px-1.5 py-0.5 transition-colors"
                         >
                           +{type}
                         </button>
                       ))}
        ...
        {/* BOTTOM ACTIONS */}
        <div className="mt-8 flex justify-end gap-3 pt-6 border-t border-gray-200 bg-white sticky bottom-0 pb-2">
          <button type="button" onClick={onClose} className="px-6 py-2.5 font-medium text-gray-600 hover:text-gray-900 hover:bg-gray-100 rounded-xl transition-colors outline-none focus:ring-2 focus:ring-gray-200">Hủy bỏ</button>
          <button type="submit" disabled={saving} className="btn-primary bg-gradient-to-r from-amber-600 to-orange-600 hover:from-amber-700 hover:to-orange-700 border-none py-2.5 px-8 rounded-xl shadow-lg shadow-amber-500/30 disabled:opacity-50 disabled:shadow-none flex items-center gap-2 outline-none focus:ring-2 focus:ring-offset-2 focus:ring-amber-500">
